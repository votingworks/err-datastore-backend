...
function convert(cognito_input) {
    const registeredVoterCount = cognito_input.PreElectionData.TotalVotersRegisteredInCity
	...
	Object.keys(pr).forEach((k) => {
		...
		combinedResults[k].candidates[new_prk] = (combinedResults[k].candidates[new_prk] || 0) + pr[k][prk]
	    })
    ...
    return {registeredVoterCount, precincts:precinctResults, contests:combinedResults}
}
